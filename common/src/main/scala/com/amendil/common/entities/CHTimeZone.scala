...
/**
  * Timezones supported by ClickHouse
  * The code can be generated with `SELECT concat('  case ', replaceRegexpAll(replaceRegexpAll(replaceRegexpAll(time_zone, '\-(\d+)', '_minus_\1'), '\+(\d+)', '_plus_\1'), '[/\-]', '_') as name, ' extends CHTimeZone("', time_zone, '")') FROM system.time_zones ORDER BY name FORMAT TSV`
  */
